...
class DCFont():
    ...
    def __init__(self, num_class, learning_rate, batch_size,vgg_path):
        self.x = tf.placeholder(dtype=tf.float32, shape=[batch_size, 224, 224, 1])
        self.num_class = num_class
        self.target = tf.placeholder(dtype=tf.float32, shape=[batch_size, 224, 224, 1])
        self.label = tf.placeholder(dtype=tf.float32, shape=[batch_size, 20])
    ...
    def discriminator(self, discrim_inputs, discrim_targets, reuse=False):
        ...
        fc1 = tf.layers.dense(tf.reshape(h3, [self.batch_size, -1]), 1)

        fc2 = tf.layers.dense(tf.reshape(h3, [self.batch_size, -1]), 20)
